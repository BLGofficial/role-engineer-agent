...
async function invokeLLM(params) {
  ...
  if (effectiveGroqKey) {
    ...
    const normalizedToolChoice2 = normalizeToolChoice(
      toolChoice || tool_choice,
      tools
    );
    if (normalizedToolChoice2) {
      payload2.tool_choice = normalizedToolChoice2;
    }
    payload2.max_tokens = maxTokens || max_tokens || 4096;
    const normalizedResponseFormat2 = normalizeResponseFormat({
      responseFormat,
      response_format,
      outputSchema,
      output_schema
    });
    if (normalizedResponseFormat2) {
      payload2.response_format = normalizedResponseFormat2;
    }
    ...
    if (!response2.ok) {
      ...
      throw new Error(`Groq API failed: ${response2.status} ${response2.statusText} \u2013 ${errorText}`);
    }
  ...
  if (ENV.geminiApiKey) {
    ...
    const generationConfig = {
      maxOutputTokens: maxTokens || max_tokens || 2048
    };
    const normalizedResponseFormat2 = normalizeResponseFormat({
      responseFormat,
      response_format,
    ...
    if (normalizedResponseFormat2 && normalizedResponseFormat2.type === "json_schema") {
      generationConfig.responseMimeType = "application/json";
      generationConfig.responseSchema = normalizedResponseFormat2.json_schema.schema;
    } else if (normalizedResponseFormat2 && normalizedResponseFormat2.type === "json_object") {
      generationConfig.responseMimeType = "application/json";
    }
    const payload2 = {
      contents,
    ...
    if (!response2.ok) {
      const errorText = await response2.text();
      throw new Error(`Gemini direct API failed: ${response2.status} ${response2.statusText} \u2013 ${errorText}`);
    }
...
// server/routers/roleEngine.ts
var roleEngineRouter = router({
  generate: publicProcedure.input(
    z2.object({
      topic: z2.string().min(1, "Topic is required"),
      masterPrompt: z2.string().min(1, "Master prompt is required")
    })
  ).mutation(async ({ input }) => {
    try {
      ...
      if (typeof content === "string") {
        ...
        text2 = content.filter((c) => "text" in c).map((c) => "text" in c ? c.text : "").join("");
      }
...
try {
  ...
  const fallbackApp = express3.default();
