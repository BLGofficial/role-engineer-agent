...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  // 2. If GEMINI_API_KEY is present, use Google Gemini API directly
  if (ENV.geminiApiKey) {
    ...
    const contents = messages.map((m) => {
      ...
      const parts = ensureArray(m.content).map((part) => {
        ...
        if (part.type === "image_url") {
          // Note: Full image support requires more complex mapping (base64)
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Gemini direct API failed: ${response.status} ${response.statusText} – ${errorText}`);
    }
  ...
  // 3. Fallback to Forge Proxy (OpenAI-style API)
  assertApiKey();

  const payload: Record<string, unknown> = {
    model: requestedModel || "gemini-2.0-flash", // Project default
    messages: messages.map(normalizeMessage),
  };
  ...
  // Thinking tokens only for supported models via forge
